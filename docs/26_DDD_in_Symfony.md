...
6. Выполняем запрос Add user v5 из Postman-коллекции v9. Видим успешный ответ, проверяем, что запись в БД создалась.
...
2. В классе `App\Domain\Command\CreateUser\Handler` исправляем метод `__invoke`
    ...
    ```
3. Выполняем запрос Add user v5 из Postman-коллекции v9. Видим в ответе идентификатор пользователя и то, что запись в
   БД создалась.
...
4. Выполняем запрос Add user v5 из Postman-коллекции v9. Видим ответ `success: false`, но запись в БД создалась.
...
7. Исправляем класс `App\Controller\Api\GetFeed\v1\Controller`
    ...
    ```
8. Выполняем запрос Add followers из Postman-коллекции v9, чтобы получить подписчиков.
9. Выполняем запрос Post tweet из Postman-коллекции v9, дожидаемся обновления лент.
10. Выполняем запрос Get feed из Postman-коллекции v9 для любого подписчика, видим твит.
11. Выполняем запрос Get feed из Postman-коллекции v9 для автора, видим пустой ответ с кодом 204.
...
2. Добавляем класс `App\Domain\ValueObject\AbstractValueObjectString`
    ...
    class AbstractValueObjectString implements ValueObjectInterface, JsonSerializable
    {
...
4. Добавляем класс `App\Doctrine\AbstractStringType`
    ...
    ```
5. В файле `config/packages/doctrine.yaml` исправляем секцию `doctrine.dbal`
    ```yaml
    dbal:
    ...
    ```
6. Исправляем класс `App\Entity\User`
    ```php
    <?php
    ...
    ```
7. В классе `App\DTO\ManageUserDTO` исправляем метод `fromEntity`
    ```php
    ...
    ```
8. В классе `App\Controller\Api\CreateUser\v5\CreateUserManager` исправляем метод `getUser`
    ```php
    ...
    ```
9. Исправляем класс `App\Domain\Command\CreateUser\CreateUserCommand`
    ```php
    <?php
    ...
    ```
10. Перезапускаем контейнер супервизора командой `docker-compose restart supervisor`
11. Выполняем запрос Add user v5 из Postman-коллекции v9. Видим, что запись в БД создалась.
12. Выполняем запрос Get users list из Postman-коллекции v9, видим ошибку
13. Очищаем кэш метаданных Doctrine командой `php bin/console doctrine:cache:clear-metadata`
14. Ещё раз выполняем запрос Get users list из Postman-коллекции v9, видим созданного пользователя.
...
2. Исправляем класс `App\Domain\Query\GetFeed\Handler`
    ...
    ```
3. Выполняем запрос Get feed из Postman-коллекции v9 для любого подписчика, видим твит.
