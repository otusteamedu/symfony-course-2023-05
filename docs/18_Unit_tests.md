...
1. В классе `UnitTests\SubscriptionServiceTest` добавляем трейт `Mockery\Adapter\Phpunit\MockeryPHPUnitIntegration`
...
13. Добавим в файл `.env.test` DSN для тестовой БД
     ```shell script
     DATABASE_URL=postgresql://user:password@postgresql_test:5432/twitter?serverVersion=15&charset=utf8
     ```
14. Выходим из контейнера и перезапускаем все контейнеры
     ```shell
     docker-compose stop
     docker-compose up -d
     ```
